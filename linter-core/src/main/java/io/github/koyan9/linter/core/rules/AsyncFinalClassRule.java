...
public final class AsyncFinalClassRule extends AbstractSpringRule {

    @Override
    public String id() {
        return "SPRING_ASYNC_FINAL_CLASS";
    }

    @Override
    public String title() {
        return "Avoid final @Async classes";
    }

    @Override
    public String description() {
        return "Final classes cannot be proxied by class-based proxies; without interfaces, @Async advice will not apply.";
    }
    ...
    public List<String> appliesWhen() {
        return List.of(
                "A final class declares `@Async` boundaries and does not implement interfaces.",
                "The application relies on proxy-based Spring async interception rather than AspectJ weaving."
        );
    }
    ...
    public List<String> recommendedFixes() {
        return List.of(
                "Remove the `final` modifier from async classes when class-based proxies are expected.",
                "Introduce an interface and expose async entrypoints through interface methods.",
                "Move async work into a separate, proxied bean if the class must remain final."
        );
    }
        ...
        for (TypeDeclaration<?> typeDeclaration : sourceUnit.structure().typeDeclarations()) {
            ...
            if (!classAsync) {
                for (MethodDeclaration method : sourceUnit.structure().methodsOf(typeDeclaration)) {
                    ...
                    if (methodFacts.hasAsyncBoundary()) {
                        methodAsync = true;
                        break;
                    }
                }
            ...
            if (classAsync || methodAsync) {
                issues.add(issue(sourceUnit, JavaSourceInspector.lineOf(typeDeclaration), "Final class '" + classDeclaration.getNameAsString() + "' declares @Async boundaries but implements no interfaces; class-based proxies cannot intercept it."));
            }
        }
        return issues;
    }
