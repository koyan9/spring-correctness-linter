...
| `spring.correctness.linter.autoDetectCentralizedSecurity` | `false` | `true` / `false` | 当源码中检测到可解析为 Spring 类型的 `SecurityFilterChain` 或 `SecurityWebFilterChain` Bean 时自动跳过 `SPRING_ENDPOINT_SECURITY`，包括组件扫描注册的实现。 |
...
| `spring.correctness.linter.autoDetectProjectWideKeyGenerator` | `false` | `true` / `false` | 当源码中检测到可解析为 Spring 类型的项目级 `@Bean KeyGenerator`、组件扫描注册的 Spring `KeyGenerator` 实现，或 `CachingConfigurer` / `CachingConfigurerSupport` 提供的 key generator 时自动跳过 `SPRING_CACHEABLE_KEY`。 |
...
如果项目统一约定了可解析为 Spring 类型的全局 `KeyGenerator` Bean、组件扫描注册的 Spring `KeyGenerator` 实现，或通过 `CachingConfigurer` / `CachingConfigurerSupport` 暴露统一 key generator，也可以显式开启自动检测：
